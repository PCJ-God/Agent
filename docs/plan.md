...
| `GET` | `/api/skills` | 可用技能（内置 + 自己的，**含正文**，前端编辑要预填） |
| `POST` | `/api/skills` | 新建自己的技能（保存即生效，见 3.14） |
| `PUT` | `/api/skills/{skill_id}` | 改自己的技能（只改传进来的字段） |
| `DELETE` | `/api/skills/{skill_id}` | 删自己的技能；内置的不在库里，删不到 |
| `GET` | `/api/mcp` | MCP **实测**状态与工具清单（只读，见 3.15） |
...
### 3.14 用户技能：存储 → API → 界面（技能终于可编辑）

承接 3.12：技能从「写死在 `skills/` 目录」变成「用户自己的数据」。

**存储层（`session_store.py`）**：新增 `user_skills` 表
（`skill_id / user_id / slug / name / description / body / created_at / updated_at`）。
它是**唯一事实来源**；磁盘上的 `data/skills/<user_id>/<slug>/SKILL.md` 只是
`materialize_user_skills()` 生成给 agentscope 读的**视图**，装配团队时重建。

为什么不用「直接往目录里写文件」：目录只能靠文件名表达归属，重命名/删除/上限校验都要
自己维护，而且并发写没有事务。库里是行，删除就是删除。

**一个必须拦的坑**：`register_agent_skill()` 遇到重名**直接抛 ValueError**
（`_toolkit.py:1381-1384`），而用户给自己的技能取名和内置技能一样是件很自然的事 ——
真抛出去会让**整个会话装配不起来**。所以在 `_merge_skill_dirs()` 里拦掉：内置优先，
重名跳过并记日志。

**API**：四个路由见上面的表。增删改之后调用 `invalidate_user_teams(user_id)`，让该用户
空闲的缓存团队失效 —— 下一次提问重新装配、重新物化，**不重启、不丢历史**。

**界面（`frontend/index.html`）**：侧栏那四行静态架构说明换成了真导航
（对话 / 技能 / MCP 服务），新功能走**抽屉**而不是新页面 —— 管工具时不该丢掉对话上下文。
技能分「内置（只读）」与「我的（可改）」；新建与编辑共用一个表单；删除是**两步确认**
（不用原生 `confirm`：它会打断页面，也说不清后果）。侧栏用一组**有语义的角色色**，
替掉"近黑底 + 单一亮蓝"那种模板感。

**文案按实测结论写**：保存后是"下一次提问就会用上"，不是"重启生效"（依据 4.13）。

### 3.15 MCP 状态：只读，而且必须是实测

先查清了现状：MCP **只有一个写死的远程服务**（`tool_manager.py` 里建
`HttpStatelessClient`，URL 取 `MCP_SERVER_URL`，实测是 DashScope 的 WebSearch）。
没有配置文件、没有启停机制、也没有"多个服务"这回事。

**`/health` 的 `mcp` 字段是有误导性的**（`server.py:242-247`）：它取自"已经装配过的
团队"，进程刚重启、还没人提问时必然是 `initializing` —— 把"MCP 连不上"和"还没人用过"
混成同一句话。所以 `/api/mcp` **真的建一次客户端、列一次工具**，状态分
`connected / unreachable / unavailable` 三档，失败时把原因原样带回（截断 300 字符）。

**只读是刻意的**：支持"自己添加 MCP 服务器"意味着要保存第三方 token，并允许 Agent 向
用户填写的任意地址发起请求（SSRF 面）。那得先定两件事 —— 谁能加、加到谁的账号下，
以及密钥存哪里。这一条**没有做**，界面上把原因写出来了，而不是留一个空占位。

**响应里不给密钥**：`mcp_server_info()` 只回主机名；`headers` 里的
`DASHSCOPE_API_KEY` 不进任何响应 —— 部署脚本里有一条断言专门查这个（4.14）。
...
- **技能抽屉同样没在真实浏览器里点过**（见 3.14）。Node + DOM 桩覆盖了列表 / 新建 /
  删除 / MCP 视图 / Escape 六条路径，但样式、真实布局与真实事件绑定仍需人点一遍。
- **技能"生效"依赖下一次团队装配**：保存后旧团队被失效，但若用户此时正有一个**在跑**的
  请求，那次请求仍然用旧工具池。本轮的端到端是"装配前保存、装配后提问"，没覆盖并发这一面。
- **删技能后清理磁盘目录没有并发保护**：用的是 `rmtree(ignore_errors=True)`，与同时在跑的
  物化可能交错；最坏结果是留下一个孤儿目录（不会被注册，只是脏）。
...
### 4.13 用户技能：34 项存储测试 + 端到端（标记词证明真的进了模型上下文）

`scripts/check_user_skills.py`（34 项全过）：

```text
跨用户隔离（最关键的一组）: bob 的列表是空的 / bob 改不了 alice 的技能 /
                          用别人的 skill_id 删 -> 删不掉
物化的路径安全:            '../../escape' 跳过、user_id '../../../etc' 跳过、
                          绕过入口直接往库里塞非法 slug -> 物化时仍然拦住
删除即时清理:              删完盘上目录立刻消失（不必等下次装配）
上限:                      第 21 个被拒（技能数量已达上限 20）
```

端到端（HTTPS，两个账号）：列表 → 新建（正文里埋标记词 `ZEBRA-7788`）→ 提问：

```text
重名 slug -> 400     非法 slug -> 400     空正文 -> 400     未认证 -> 401
C 看到的用户技能数 -> 0        C 删 A 的技能 -> 404      ← 隔离
提问回答 : "ZEBRA-7788 的答案是紫色。"
日志     : "name": "read_skill_file"  ×2
```

**标记词只存在于用户自建的技能正文里** —— 这句话能答出来，就证明用户技能真的进了模型
上下文，而不只是存进了数据库。删完 `user_skills` 0 行，其它表行数不变。

**期间闸门抓到的两个真问题**（都不是"看着像成功"能发现的）：

1. **删技能后磁盘留了孤儿 `SKILL.md`**：清理原本只在装配团队时顺带做。行为上无害
   （清单来自库，孤儿不会被注册），但看着像"删不干净"，改成删除时立刻清。
2. **列表接口没带正文**（`_SKILL_COLS` 里没有 `body`）：前端"编辑"会把正文预填成空白，
   改一个错字就得全文重打。部署闸门里那条"列表里带正文 >= 1"的断言抓到的，已修，
   并在存储测试里补了对应断言（原来那条断言写的是"不带正文"，正好把错误假设固化了）。

### 4.14 前端导航/抽屉（15 项）+ MCP 实测

`scripts/check_frontend.js`（三层 + 事件驱动）：

```text
1) 顶层执行不抛异常                                  ✓
2) 23 个 getElementById 的 id 在 HTML 里都存在        ✓
3) 账号面板：绑定 / 登录（含换账号）/ 退出 / 关闭       ✓
4) 侧栏 + 抽屉：技能列表、数量、表单四字段、提交 POST、
   两步删除 → DELETE、MCP 视图、Escape 关闭、高亮复位   ✓
```

桩这轮升级成**能记录并触发事件**（`addEventListener` 存回调 + `fire()` 驱动，
`querySelector` 对同一父元素返回同一个孩子）—— 否则"点按钮"只能靠直接调函数，
测不到真实绑定路径。升级过程中桩自己有个 bug（`__attrs` 没初始化，`setAttribute`
抛异常）导致 6 条级联失败，修的是桩不是产品代码 —— 这类失败要区分清楚，否则会改错地方。

**MCP 实测**（补上第六节「MCP 从未被真实触发」一直缺的那半）：

```text
{"status":"connected","name":"web_search_service","transport":"streamable_http",
 "host":"dashscope.aliyuncs.com","tools":["bailian_web_search"]}
未认证 -> 401
响应里没有 sk-/Bearer/api_key/authorization 痕迹   ✓
```

**但要说清验证到哪一步为止**：这证明**服务可达、工具名可枚举**；
「模型在一段真实对话里主动调用 `bailian_web_search`」**仍未验证**。

**前端一致性**：本地 / 服务器上的文件 / HTTPS 实际返回，三份 md5 相同
（`44bcbf4ee592b61a0bb92d7369447126`）。

**部署闸门的顺序**（每一步都是被真实故障教出来的）：导入冒烟（502 那次就是漏了这步）→
跑测试 → 重启 → **等到真的能响应**（不是 `systemctl is-active` 就算完）→ 逐字节核对 →
安全断言 → 未认证必须 401。
...
- [~] **MCP：本轮补上了一半**。原判断是「`/health` 的 `mcp` 一直是 `initializing`，
      两次真实对话都没走联网搜索」。其中一半是被那个字段误导的 —— 它只看"已经装配过的
      团队"，进程刚重启、还没人提问时必然显示 `initializing`。改用实测接口后
      （见 3.15 / 4.14）：服务可达、只暴露一个工具 `bailian_web_search`、响应不含密钥。
      **仍未验证**：「模型在一段真实对话里主动调用这个工具」。
